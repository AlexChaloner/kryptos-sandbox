...
test("wheel zoom is pointer anchored and difference views are non-destructive", () => {
  ...
  assert.match(app, /function gridGeometryFor\(grid\)/);
  assert.match(html, /data-difference-axis="horizontal"/);
  assert.match(html, /data-difference-axis="vertical"/);
  assert.match(styles, /\.letter-cell\.difference-cell\s*\{\s*font-size:\s*10px/);
  assert.match(app, /const columnSizes = gridTrackSizes\(geometry\.sourceColumns, grid\.cellSize, modes\.horizontal\)/);
  assert.match(app, /const differenceSize = differenceCellSize\(grid\.cellSize\)/);
  assert.match(app, /const startGeometry = gridGeometryFor\(grid\)/);
  assert.match(app, /const resize = columnsForResizeDrag\(\{/);
  assert.match(app, /if \(resize\.axis !== mode\)/);
  assert.match(app, /reshape\(resize\.columns\)/);
